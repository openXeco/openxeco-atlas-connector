...
describe('ATLAS cluster retrieval', () => {
  ...
  it('does not query configuration or ATLAS for a blank registration number', async () => {
    await expect(getClustersByRegistrationCode('   ', [], atlas.client)).resolves.toEqual([])
  ...
  it('filters by registration number and configured country and excludes locally linked entities', async () => {
    ...
    await expect(
      getClustersByRegistrationCode('LU-123', ['atlas-linked'], atlas.client),
    ).resolves.toEqual([
      expect.objectContaining({ id: 'atlas-available', atlasId: 'atlas-available' }),
    ])

    expect(getSettingMock).toHaveBeenCalledWith(SETTINGS_KEYS.COUNTRY, expect.anything())
  ...
  it('returns an empty list when ATLAS has no matching data', async () => {
    ...
    await expect(getClustersByRegistrationCode('LU-123', [], atlas.client)).resolves.toEqual([])
  })
})
