...
describe('findCorrespondences', () => {
  ...
  it('excludes entities already linked to ATLAS', async () => {
    ...
    await expect(findCorrespondences('LU-123', atlasClient, db, logger)).resolves.toBe(matches)

    expect(getEntitiesIdWithAtlasIdMock).toHaveBeenCalledWith(db)
    expect(getClustersByRegistrationCodeMock).toHaveBeenCalledWith('LU-123', excludedIds, atlasClient, db, logger)
  })
})
