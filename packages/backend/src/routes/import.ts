      ...
      try {
        ...
        try {
          ...
          return reply.status(401).send({
            ...
            message: message.includes('Invalid credentials')
              ? 'Invalid email or password for cybersecurity.lu'
              : message,
          });
        }
        ...
        if (!answers || answers.length === 0) {
          return reply.status(404).send({
            error: 'No Data Found',
            message: 'No form answers found for this account. Please complete the ECCC registration form on cybersecurity.lu first.',
          });
        }
        ...
        return reply.send({
          ...
          message: result.errors.length === 0
            ? 'Form data imported successfully'
            : 'Form data imported with some errors',
        });
      } catch (error) {
